<script lang="ts">
	...
	let { data = [], labels = [], yTicks = [], yLabel = '' } = $props();
	...
	<svg {width} {height} class="bg-background">
		<g class="bars">
			{#each data as { value, color }, i}
				<Rect {value} {xScale} {yScale} {barWidth} {color} {i} animate={chartVisible} />
			{/each}
		</g>
		...
		<g class="axis y-axis text-xs">
			{#each yTicks as tick}
				<g class="tick tick-{tick}" transform="translate(0, {yScale(tick)})">
					<line x2="100%" stroke="currentColor" class="text-accent-foreground/50" />
					<text x="1" y="-4" fill="currentColor" class="text-accent-foreground/80">
						{tick}
						{tick === yTicks[yTicks.length - 1] ? yLabel : ''}
					</text>
				</g>
			{/each}
		</g>
