...
<nav
	class="fixed top-0 left-0 right-0 z-50 shadow-md bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80"
>
	{#if showNotification}
		<div class="bg-primary/90">
			<div
				class="md:container md:mx-auto p-2 flex h-auto items-center justify-between text-primary-foreground"
			>
				<p>
					{$t('nav.launchEvent')}
					<a
						href="https://forms.gle/bbCVEsG4f319weMT9"
						class="underline hover:text-accent-foreground/60 duration-300"
						target="_blank"
						rel="noopener noreferrer">{$t('nav.rsvpHere')}</a
					>
				</p>
	...
	<div class="md:container md:mx-auto px-2 md:px-4 flex h-16 items-center justify-between">
		<div class="flex items-center gap-4">
			...
			<a href="/" class="px-1">
				<img
					src={mode.current === 'dark' ? '/logo_dark.png' : '/logo_light.png'}
					alt="MaryaNutri"
					class="h-14 md:h-16 w-auto py-2"
				/>
			</a>
			...
			<Dialog.Root bind:open={dialogOpen}>
				<Dialog.Trigger class="mx-1">
					...
					{#snippet child({ props })}
						<div
							{...props}
							class={`${buttonVariants({ variant: 'default', size: 'default' })} text-sm md:text-base px-3 md:px-4`}
						>
				...
				<Dialog.Content>
					...
					<form class="space-y-4" onsubmit={handleSubmit}>
						...
						{#if submitStatus === 'success'}
							<div
								class="p-3 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded-md"
							>
			...
			<!-- Socials -->
			<a
				href="https://www.facebook.com/maryanutri"
				class="hidden md:flex duration-300 hover:text-primary"
				target="_blank"
			...
			<a
				href="https://www.linkedin.com/company/maryanutri"
				class="hidden md:flex duration-300 hover:text-primary"
				target="_blank"
				rel="noopener noreferrer"
				aria-label="Linkedin"
				...
				<li>
					<div class="px-2 lg:px-3">
						...
						>
							...
							<span
								class="block w-full h-0.5 bg-primary/75 transform origin-center scale-x-0 transition-transform duration-500 group-hover:scale-x-125"
								class:scale-x-125={$page.url.pathname === item.href}
							></span>
						</a>
					</div>
				</li>
				...
				<DropdownMenu.Content>
					<DropdownMenu.Group>
						...
						<DropdownMenu.Item
							...
							class={currentLocale === 'nl' ? 'bg-accent' : ''}
						>
							<span class="mr-1">🇳🇱</span> Dutch
						</DropdownMenu.Item>
						<DropdownMenu.Item
							onclick={() => switchLanguage('en')}
							class={currentLocale === 'en' ? 'bg-accent' : ''}
						>
							<span class="mr-1">🇬🇧</span> English
						</DropdownMenu.Item>
						<DropdownMenu.Item
							onclick={() => switchLanguage('fr')}
							class={currentLocale === 'fr' ? 'bg-accent' : ''}
						>
							<span class="mr-1">🇫🇷</span> French
						</DropdownMenu.Item>
						<DropdownMenu.Item
							onclick={() => switchLanguage('sw')}
							class={currentLocale === 'sw' ? 'bg-accent' : ''}
						>
					...
					<div class="flex flex-col gap-4">
						<DropdownMenu.Root>
							<DropdownMenu.Trigger class="flex">
								...
								<span class="text-accent-foreground font-normal ml-3 my-auto"
									>{$t('common.switchTheme')}</span
								>
							</DropdownMenu.Trigger>
							<DropdownMenu.Content>
								<DropdownMenu.Item onclick={() => setMode('light')}>
...
<main class:mt-24={showNotification} class:mt-16={!showNotification}>
	{@render children()}
</main>
...
<footer class="bg-accent py-10 pb-8">
	<div class="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
		...
		<div>
			...
			<ul class="space-y-2">
				...
				<li>
					<a href="/products" class="duration-300 hover:text-primary">{$t('nav.products')}</a>
				</li>
	...
	<div class="text-center mt-8 text-sm">
		{$t('common.madeWith')}
		<a href="https://svelte.dev/" target="_blank" rel="noopener noreferrer">❤️</a>
		{$t('common.by')}
		<a
			class="font-mono duration-300 hover:text-destructive"
			href="https://github.com/robsyc"
			target="_blank"
			rel="noopener noreferrer">robsyc</a
		>
	</div>
</footer>
