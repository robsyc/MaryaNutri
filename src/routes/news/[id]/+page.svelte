...
<div class="container mb-12">
	...
	<article class="max-w-4xl mx-auto">
		...
		<header class="mb-8">
			{#if article.category}
				<span
					class="inline-block bg-primary/10 text-primary text-sm font-semibold px-3 py-1 rounded mb-4"
				>
					{article.category}
				</span>
			{/if}
			...
			{#if article.tags && article.tags.length > 0}
				<div class="flex flex-wrap gap-2 mb-6">
					{#each article.tags as tag}
						<span
							class="inline-flex items-center bg-accent/20 text-accent-foreground text-xs px-2 py-1 rounded"
						>
		...
		<!-- Article Image -->
		<div class="mb-8">
			<img src={article.image} alt={article.title} class="w-full rounded-lg shadow-lg" />
		</div>
			...
			<div class="bg-primary/5 border border-primary/20 p-6 rounded-lg">
				{#if article.category === 'Article'}
					...
					<p class="text-muted-foreground mb-4">
						This article was originally published externally. Click below to read the complete
						story.
					</p>
				...
				{:else if article.category === 'Media'}
					...
					<p class="text-muted-foreground mb-4">
						This video was originally published externally. Click below to watch the complete video.
					</p>
					<a href={article.externalLink} target="_blank" rel="noopener noreferrer">
						<Button class="inline-flex items-center">
							<ExternalLink class="h-4 w-4 mr-1" />
				...
				{:else if article.category === 'Event'}
					...
					<p class="text-muted-foreground mb-4">
						Register or get more information about this event.
					</p>
					<a href={article.externalLink} target="_blank" rel="noopener noreferrer">
						<Button class="inline-flex items-center">
							<ExternalLink class="h-4 w-4 mr-1" />
